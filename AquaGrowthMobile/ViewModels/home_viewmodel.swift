...
class home_viewmodel: ObservableObject {
                ...
                if let error = error {
                    ...
                    self.favoritePlants = querySnapshot?.documents.compactMap { document in
                        ...
                        if let plantImagePath = data["plant_image"] as? String {
                            self.loadImageFromFirebase(plantId: UUID(uuidString: plantID)!, imagePath: plantImagePath)
                        }
    ...
    private func updatePlantImage(plantId: UUID, image: UIImage) {
        if let index = self.favoritePlants.firstIndex(where: { $0.id == plantId }) {
            self.favoritePlants[index].plant_ui_image = image
        }
    }
